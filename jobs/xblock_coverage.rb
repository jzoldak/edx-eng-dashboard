...
SCHEDULER.every '5m', :first_in => 0 do |job|
    ...
    last_build_uri = '/job/xblock-master-branch-tests/lastStableBuild/buildNumber'
    response = http.request(Net::HTTP::Get.new(last_build_uri))
    last_build_num = response.body.to_i

    # Retrieve the coverage info as JSON
    coverage_uri = "/job/xblock-master-branch-tests/#{last_build_num}/cobertura/api/json?depth=2"
    ...
    send_event('xblock_coverage', { value: line_coverage })
end
